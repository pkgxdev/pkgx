...
export function is_unsafe(unsafe: boolean): boolean {
  ...
  const IS_UNSAFE = parseInt(Deno.env.get("PKGX_UNSAFE_INSTALL") || "0")
    ? true
    : unsafe;
  return IS_UNSAFE;
}
  ...
  async function write(dst: Path, pkgs: PackageRequirement[]) {
    ...
    for (const pkg of pkgs) {
      ...
      for (const program of programs) {
        ...
        if (f.exists()) {
          if (!f.isFile()) {
            throw new PkgxError(`${f} already exists and is not a file`);
          }

          const fd = is_shebang(f).catch(() => undefined);
          if (!fd) {
            throw new PkgxError(
              `${f} already exists and is not a pkgx installation`,
            );
          }
          ...
          while (true) {
            ...
            if (found) {
              n++;
              console.warn(
                `pkgx: already installed: ${blurple(program)} ${
                  dim(`(${found})`)
                }`,
              );
