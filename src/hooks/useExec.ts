...
async function install(pkgs: PackageSpecification[], update: boolean) {
  const { modifiers: { json, dryrun, verbosity }, env } = useConfig()
  ...
  for (const pkg of pending) {
    ...
    if (dryrun) {
      install = { pkg, path: usePrefix().join(pkg.project, `v${pkg.version}`) }
      log_install_msg(pkg, 'imagined')
    } else {
      let bytes = 0
      const timestamp = Date.now()
      install = await base_install(pkg, {
        locking: () => {
          if (!json) {
...
import { usePrefix } from "./index.ts";
